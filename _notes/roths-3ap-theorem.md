...
<h1>Density Increment Argument</h1>
...
$$\alpha = \frac{|A \cap [N]|}{N} \leq \frac{|A \cap P|}{|P|} - \Omega(\alpha^2)$$
...
$$(\alpha + \Omega(\alpha^2)) \ |P| \leq |A \cap P|$$

In order to establish such an inequality, it suffices to show that

$$\sum_{i=1}^k \Omega(\alpha^2) |P_i| \leq \sum_{i=1}^k \left||A \cap P_i| - \alpha P_i\right|$$

The right-hand side can be rewritten as

$$\sum_{i=1}^k \left| \sum_{x \in P_i} (\mathbb{I} - \alpha)(x)\right| \text{   (1)}$$
...
<br>
<h1>Strategy and Pre-Requisites</h1>
<br>
...
<div class="card mt-3 p-3">
  <h3 class="card-title font-weight-medium">Lemma 1.</h3>
  <div class="card-text">
...
<h3>3AP-free $\Rightarrow$ large Fourier coefficient</h3>
...
<br>
<div class="card mt-3 p-3">
  <h3 class="card-title font-weight-medium">Lemma 2 (Counting Lemma).</h3>
  <div class="card-text">
    <p>Let $f, g : \ \mathbb{Z} \rightarrow \mathbb{C}$ and $\sum_{n \in \mathbb{Z}} |f(n)|^2 \leq M, \sum_{n \in \mathbb{Z}} |g(n)|^2 \leq M$, for some $M$. Then, we have that:</p>

    $$\left| \Lambda_3(f) - \Lambda_3(g) \right| < 3M \Vert \widehat{f - g}\Vert_{\infty}$$
  </div>
</div>
<br>
...
$$|\Lambda_3(f) - \Lambda_3(g)| \leq |\Lambda(f - g, f, f)| + |\Lambda(g, f - g, f)| + |\Lambda(g, g, f - g)| \leq 3M \Vert \widehat{f - g}\Vert_{\infty} \square$$

Now, we continue with finding our large Fourier coefficient.

<br>
<div class="card mt-3 p-3">
  <h3 class="card-title font-weight-medium">Lemma 3 (No 3AP $\Rightarrow$ Non-uniformity).</h3>
  <div class="card-text">
    <p>Let $A \subset [N]$ be 3-AP free with $|A| = \alpha N$ and $N \geq 5/\alpha^2$. Then there exists some $\theta$ such that </p>

    $$\left|\sum _{x \in \mathbb{Z}}^{N}(\mathbb{I}_{A}-\alpha )(x)e(-x\theta)\right|\geq {\frac {\alpha ^{2}}{10}}N$$.
  </div>
</div>
<br>
...
<h1>Large Fourier Coefficient $\Rightarrow$ Density Increment</h1>
...
<br>
<div class="card mt-3 p-3">
  <h3 class="card-title font-weight-medium">Lemma 4 (Dirichlet).</h3>
  <div class="card-text">
...
<br>
<div class="card mt-3 p-3">
  <h3 class="card-title font-weight-medium">Lemma 5 (Dirichlet).</h3>
  <div class="card-text">
    <p>Let $ 0<\eta <1,\theta \in \mathbb {R} $. Assume that $ N>C\eta ^{-6}$ for a universal constant $C$. Then it is possible to partition $[N]$ into arithmetic progressions $ P_{i}$ with length $ N^{1/3}\leq |P_{i}|\leq 2N^{1/3}$ such that $ \sup _{x,y\in P_{i}}|e(x\theta )-e(y\theta )|<\eta $ for all $ i$. </p>
  </div>
</div>
<br>
    
*Proof*. We first bound $|e(x\theta) - e(y\theta)|$ for $x, y \in P_i$. Because they lie in am arithmetic progression with length $d$, let $x = y + kd$ for some $k$. We can start by rewriting this difference as a telescoping sum

$$|e(x\theta) - e(y\theta)| = \left|\sum_{i = 1}^k e((y + id)\theta) - e((y + (i - 1)d)\theta)\right| \leq \sum_{i = 1}^k  |e((y + id)\theta) - e((y + (i - 1)d)\theta)|$$

$$ = \sum_{i = 1}^k |e(d\theta) - 1| \leq |P_i| \  |e(d\theta) - 1|$$

Thus, it suffices to pick a common difference $d$ that makes $e(d\theta)$ arbitrarily close to 1. We use the above lemma to choose a $d$ such that $d\theta \leq \frac{\eta}{3\pi N^{1/3}}$ where $d \leq \frac{4\pi N^{1/3}}{\eta}$. First, notice that $|e(d\theta) - 1|$ is bounded by the arc length of $d\theta$ degrees. Thus,

$$|e(d\theta) - 1| \leq 2\pi \left( \frac{\eta}{4 \pi N^{1/3}}\right) = \frac{\eta}{2N^{1/3}} $$
Thus,

$$\sup _{x,y\in P_{i}}|e(x\theta )-e(y\theta )| \leq |P_i| \ |e(d\theta) - 1| \leq 2N^{1/3} \left( \frac{\eta}{2 N^{1/3}}\right) = \eta$$
...
<div class="card mt-3 p-3">
  <h3 class="card-title font-weight-medium">Lemma 6 (Non-uniformity $\Rightarrow$ Density Increment).</h3>
  <div class="card-text">
    <p>Let $A$ be a 3-AP-free subset of $[N]$, with $|A|=\alpha N$ and $N > C\alpha ^{-12}$. Then, there exists a sub progression $P\subset [N]$ such that $|P|\geq N^{1/3}$ and $|A\cap P|\geq (\alpha +\alpha ^{2}/40)|P|$. </p>
  </div>
</div>
<br>
...
$$\frac{\alpha^2}{10}N \leq \left|\sum _{x \in \mathbb{Z}}^{N}(\mathbb{I}_{A}-\alpha )(x)e(-x\theta)\right| = \left|\sum_{i = 1}^k \sum _{x \in P_i}(\mathbb{I}_{A}-\alpha )(x)e(-x\theta)\right| \leq \sum_{i = 1}^k \left| \sum _{x \in P_i}(\mathbb{I}_{A}-\alpha )(x)e(-x\theta)\right|$$

and because $\sup _{x,y\in P_{i}}|e(x\theta )-e(y\theta )|<\frac{\alpha^2}{20}$,

$$\leq \sum_{i = 1}^k \left(\left| \sum _{x \in P_i}(\mathbb{I}_{A}-\alpha )(x)e(-y_i\theta)\right| + \left| \sum _{x \in P_i}(\mathbb{I}_{A}-\alpha )(x)(e(-x\theta) - e(-y_i\theta))\right|\right)$$

$$\leq \sum_{i = 1}^k\left( \left| \sum _{x \in P_i}(\mathbb{I}_{A}-\alpha )(x)\right| + \frac{\alpha^2}{20}|P_i|\right)$$

for some fixed $y_i \in P_i$. And we have that

$$\frac{\alpha^2}{20} \sum_{i = 1}^k |P_i| = \frac{\alpha^2}{20}N \leq \sum_{i = 1}^k \left| \sum _{x \in P_i}(\mathbb{I}_{A}-\alpha )(x)\right| = \sum_{i = 1}^k \left| |A \cap P_i | - \alpha P_i \right|$$

$$=\sum_{i = 1}^k \left| |A \cap P_i | - \alpha P_i \right| + |A\cap P_i| - \alpha P_i$$

Notice that $\left| |A \cap P_i | - \alpha P_i \right| + |A\cap P_i| - \alpha P_i$ is either 0 or $2|A\cap P_i| - 2\alpha P_i$. Because the right hand side is larger than the left, at least one term on the right is at least as large as a term on the left, and since all terms on the left sum are greater than 0, we have that there is some $P_i$ such that

$$\frac{\alpha^2}{20} |P_i| \leq 2|A\cap P_i| - 2\alpha P_i$$

$$|A\cap P_i|\geq (\alpha +\alpha ^{2}/40)|P_i| \square$$
